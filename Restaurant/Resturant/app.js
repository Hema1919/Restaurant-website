...
function generteItem(items){
        return `<div class="menu-item col-md-6 col-lg-4">
        <div class="card">
          <img src="${items.img}" class="card-img-top" alt="${items.title}">
          <div class="card-body">
            <h5 class="card-title">${items.title}</h5>
            <h6 class="card-subtitle mb-2">${items.price}</h6>
            <p class="card-text">${items.description}</p>
            <p class="card-text"><small class="text-muted">Category: ${items.category}</small></p>
          </div>
        </div>
      </div>`;
}
...
function dispalyItem(menu){
  const menuContainer = document.getElementById('menuItems');
  menuContainer.innerHTML = '';
  menu.forEach(element => {
    const menuItemElement = generteItem(element);
    menuContainer.innerHTML += menuItemElement;
...
const startersElement = document.querySelector('.js-starters');
const allElement = document.querySelector('.js-all');
const mainsElement = document.querySelector('.js-mains');
const dessertElement = document.querySelector('.js-desserts');
const beverageElement = document.querySelector('.js-beaverges');
...
searchElement.addEventListener('keyup',(event)=>{  
  ...
  menu.forEach((element) => {
    if(element.category.indexOf(a) !== -1){
    const menuItemElement = generteItem(element); 
    menuContainer.innerHTML += menuItemElement;
    }
  })
...
startersElement.addEventListener('click',()=>{
  const menuContainer = document.getElementById('menuItems');
  menuContainer.innerHTML = '';  
  menu.forEach((element) => {   
...
allElement.addEventListener('click',()=>{
  ...
  menuContainer.innerHTML = '';  
  menu.forEach((element) => {   
  if (element.category === 'starters' || element.category ==='beverages' || element.category ==='mains' || element.category ==='desserts' ){   
    const menuItemElement = generteItem(element);    
    menuContainer.innerHTML += menuItemElement;
  }
});   
...
mainsElement.addEventListener('click',()=>{
  const menuContainer = document.getElementById('menuItems');
  menuContainer.innerHTML = '';  
  menu.forEach((element) => {   
...
beverageElement.addEventListener('click',()=>{
  const menuContainer = document.getElementById('menuItems');
  menuContainer.innerHTML = '';  
  menu.forEach((element) => {   
